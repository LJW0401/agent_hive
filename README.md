...
- **多终端并行** — 每个项目支持最多 5 个终端标签页，默认终端（Agent）始终保持连接
- **项目容器** — 每个容器包含独立终端组 + 待办清单，可编辑名称
- **双布局模式** — 桌面端支持多项目（2×2 网格）和单项目（全屏）两种布局，一键切换，状态持久化
- **可调布局** — 桌面端/移动端均可拖拽分割线调整待办/终端比例
- **终端懒加载** — 默认终端（Agent）始终保持连接，额外终端仅在激活时加载，切换离开后释放
- **完整终端模拟** — 支持 ANSI 颜色、vim、htop 等 TUI 程序（基于 xterm.js）
- **移动端适配** — 滑动切换项目、7/3 默认布局（终端在上）、快捷键栏（^C/^L/Tab/Esc/方向键/Enter）、粘贴功能
- **多设备同步** — 终端输入输出、终端创建/关闭、待办事项、容器操作在所有设备间实时同步
- **会话持久化** — 关闭浏览器后终端输出历史和待办数据不丢失，服务重启后可恢复（默认终端 1000 行，额外终端 200 行）
- **关闭确认** — 关闭有运行中进程的终端时弹出确认对话框
...
| 存储 | SQLite（容器/布局/待办/终端元数据）, 磁盘文件（终端历史、运行日志）|
| 测试 | Go test, Vitest |
...
- **Agent**（默认终端）— 始终保持连接，不可关闭，历史保留 1000 行
- **Terminal 2~5**（额外终端）— 按需创建，懒加载，关闭有进程时需确认，历史保留 200 行

标签栏 "+" 按钮新建终端，"x" 按钮关闭终端（仅额外终端），终端数达 5 个时隐藏 "+"。

### 布局模式

桌面端支持两种布局模式，通过顶部栏按钮切换：

- **多项目模式**（默认）— 2×2 网格，一页显示 4 个项目，支持分页和拖拽排序
- **单项目模式** — 一个项目占满页面，标题栏左右箭头或 `Ctrl+←/→` 切换项目，带滑动动画

布局状态通过 localStorage 持久化，刷新后保持。
...
│       ├── config/          # 配置加载（含用户推断）
│       ├── container/       # 容器管理（Container → Terminal 多终端模型）
│       ├── logger/          # 日志文件写入与轮转
│       ├── pty/             # PTY 会话（含用户切换）
│       ├── server/          # HTTP 路由与 API（含终端 CRUD + 进程检测）
│       ├── static/          # go:embed 前端资源
│       ├── store/           # SQLite 存储（容器/终端/布局/待办）
│       └── ws/              # WebSocket 处理（支持 tid 参数路由到具体终端）
├── frontend/
│   └── src/
│       ├── components/      # React 组件
│       │   ├── Terminal.tsx          # 终端（支持懒加载）
│       │   ├── TerminalTabBar.tsx    # 终端标签栏（桌面+移动共用）
│       │   ├── SingleProjectView.tsx # 单项目全屏视图
│       │   ├── ProjectContainer.tsx  # 多项目网格容器
│       │   ├── ConfirmDialog.tsx     # 关闭确认弹窗
│       │   ├── ShortcutBar.tsx       # 移动端快捷键栏
│       │   ├── PasteModal.tsx        # 粘贴弹窗
│       │   └── ...
│       ├── api.ts           # API 调用（含终端 CRUD）
│       ├── App.tsx          # 桌面端应用（双布局模式）
│       └── MobileApp.tsx    # 移动端应用
